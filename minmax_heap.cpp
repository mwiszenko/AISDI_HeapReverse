...
namespace {
    ...
    public:
        ...
        /**
         * Changes the direction of the heap (min <-> max) in O(n) time.
         */
        void reverse() {
...
double reverseHeapAndSaveResultsToFile( std::vector<int> &v, int size, std::ofstream &file ) {
    file << size << " ";
    ...
    auto heap = ReversibleHeap<int>();
    test::fill_heap( heap, vCopy );

    // measure time of reversing heap
    Benchmark<std::chrono::nanoseconds> b;
    heap.reverse();
    long elapsed = b.elapsed();
...
int main() {
    std::ofstream file;
    file.open( "reverseHeap-results.txt" );

    // Create heap of size, reverse it and save results to file
    std::vector<int> v1;
    for ( int i = 100; i <= 10000; i += 100 ) {
        reverseHeapAndSaveResultsToFile( v1, i, file );
    }
    ...
    // Example vector
    std::vector<int> v{ 41, 15, 20, 25, 30, 35, 40, 45, 50, 55, 60, 65, 70, 75, 80 };
